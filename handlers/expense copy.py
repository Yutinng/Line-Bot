...
from decimal import Decimal  # 確保 Decimal 轉換處理
...
# 儲存記帳資料
def save_expense(category, sub_category, amount, user_id):
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute(
        "INSERT INTO expenses (category, sub_category, amount, user_id) VALUES (%s, %s, %s, %s)",
        (category, sub_category, amount, user_id)
    )
    connection.commit()
    cursor.close()
    connection.close()
